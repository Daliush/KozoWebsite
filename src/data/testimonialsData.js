import elyfroThumbnail from '../images/Elyfro_thumbnail.png'
import elyfro1 from '../images/Elyfro_1.png'
import elyfro2 from '../images/Elyfro_2.png'
import elyfro3 from '../images/Elyfro_3.png'
import elyfro4 from '../images/Elyfro_4.png'
import fiansoThumbnail from '../images/Fianso_Thumbnail.png'
import fianso1 from '../images/Fianso_1.png'
import fianso2 from '../images/Fianso_2.png'
import fianso3 from '../images/Fianso_3.png'
import fianso4 from '../images/Fianso_4.png'
import fianso5 from '../images/Fianso_5.png'
import lightThumbnail from '../images/Light_Thumbnail.png'
import light1 from '../images/Light_1.png'
import light2 from '../images/Light_2.png'
import kheopsThumbnail from '../images/Kheops_Thumbnail.png'
import kheops1 from '../images/Kheops_1.png'
import kheops2 from '../images/Kheops_2.png'
import kheops3 from '../images/Kheops_3.png'
import Driak21Thumbnail from '../images/Driak21_thumbnail.png'
import Driak211 from '../images/Driak21_1.png'
import Driak212 from '../images/Driak21_2.png'
import Driak213 from '../images/Driak21_3.png'
import Driak214 from '../images/Driak21_4.png'
import Driak215 from '../images/Driak21_5.png'
import frenchFlag from '../images/frenchFlag.png'
import ElToredoThumbnail from '../images/ElToredo_Thumbnail.png'
import ElToredo1 from '../images/ElToredo_1.png'
import ElToredo2 from '../images/ElToredo_2.png'
import ElToredo3 from '../images/ElToredo_3.png'
import NexoThumbnail from '../images/Nexo_thumbnail.png'
import Nexo1 from '../images/Nexo_1.png'
import Nexo2 from '../images/Nexo_2.png'
import Nexo3 from '../images/Nexo_3.png'
import Nexo4 from '../images/Nexo_4.png'
import Nexo5 from '../images/Nexo_5.png'
import Nexo6 from '../images/Nexo_6.png'
import Nexo7 from '../images/Nexo_7.png'
import Nexo8 from '../images/Nexo_8.png'
import PsykoThumbnail from '../images/Psyko_thumbnail.png'
import Psyko1 from '../images/Psyko_1.png'
import Psyko2 from '../images/Psyko_2.png'
import Psyko3 from '../images/Psyko_3.png'
import YoshiThumbnail from '../images/Yoshi_thumbnail.png'
import Yoshi1 from '../images/Yoshi_1.png'
import Yoshi2 from '../images/Yoshi_2.png'
import Yoshi3 from '../images/Yoshi_3.png'
import Yoshi4 from '../images/Yoshi_4.png'
import Yoshi5 from '../images/Yoshi_5.png'
import ukFlag from '../images/ukFlag.png'
import BelbarbThumbnail from '../images/Belbarb_Thumbnail.png'
import Belbarb1 from '../images/Belbarb_1.png'
import Belbarb2 from '../images/Belbarb_2.png'
import Belbarb3 from '../images/Belbarb_3.png'
import Belbarb4 from '../images/Belbarb_4.png'
import Belbarb5 from '../images/Belbarb_5.png'
import NelwyrThumbnail from '../images/Nelwyr_Thumbnail.png'
import Nelwyr1 from '../images/Nelwyr_1.png'
import Nelwyr2 from '../images/Nelwyr_2.png'
import Nelwyr3 from '../images/Nelwyr_3.png'
import Nelwyr4 from '../images/Nelwyr_4.png'
import MaleloThumbnail from '../images/Malelo_Thumbnail.png'
import Malelo1 from '../images/Malelo_1.png'
import Malelo2 from '../images/Malelo_2.png'
import Malelo3 from '../images/Malelo_3.png'
import Malelo4 from '../images/Malelo_4.png'
import Malelo5 from '../images/Malelo_5.png'
import Malelo6 from '../images/Malelo_6.png'
import Malelo7 from '../images/Malelo_7.png'
import CerttesThumbnail from '../images/Certtes_Thumbnail.png'
import Certtes1 from '../images/Certtes_1.png'
import Certtes2 from '../images/Certtes_2.png'
import Certtes3 from '../images/Certtes_3.png'
import Certtes4 from '../images/Certtes_4.png'
import Certtes5 from '../images/Certtes_5.png'
import HighqueueThumbnail from '../images/HighQueue_Thumbnail.png'
import Highqueue1 from '../images/Highqueue_1.png'
import Highqueue2 from '../images/Highqueue_2.png'
import MugenThumbnail from '../images/Mugen_Thumbnail.png'
import Mugen1 from '../images/Mugen_1.png'
import Mugen2 from '../images/Mugen_2.png'
import Mugen3 from '../images/Mugen_3.png'
import KkhumaThumbnail from '../images/Kkhuma_Thumbnail.png'
import KKhuma1 from '../images/Kkhuma_1.png'
import KKhuma2 from '../images/Kkhuma_2.png'
import KKhuma3 from '../images/Kkhuma_3.png'
import KKhuma4 from '../images/Kkhuma_4.png'
import KKhuma5 from '../images/Kkhuma_5.png'
import DeadcrafteurThumbnail from '../images/Deadcrafteur_Thumbnail.png'
import Deadcrafteur1 from '../images/Deadcrafteur_1.png'
import Deadcrafteur2 from '../images/Deadcrafteur_2.png'
import Deadcrafteur3 from '../images/Deadcrafteur_3.png'
import Deadcrafteur4 from '../images/Deadcrafteur_4.png'
import Deadcrafteur5 from '../images/Deadcrafteur_5.png'
import Deadcrafteur6 from '../images/Deadcrafteur_6.png'

export const testimonialsData = [
    {
        name: 'NeXo',
        flag: frenchFlag,
        flagAlt: 'french',
        thumbnail: NexoThumbnail,
        images: [
            { src: Nexo1, alt: 'nexo1' },
            { src: Nexo2, alt: 'nexo2' },
            { src: Nexo3, alt: 'nexo3' },
            { src: Nexo4, alt: 'nexo4' },
            { src: Nexo5, alt: 'nexo5' },
            { src: Nexo6, alt: 'nexo6' },
            { src: Nexo7, alt: 'nexo7' },
            { src: Nexo8, alt: 'nexo8' }
        ]
    },
        {
        name: 'Kkhuma',
        flag: frenchFlag,
        flagAlt: 'french',
        thumbnail: KkhumaThumbnail,
        images: [
            { src: KKhuma1, alt: 'Kkhuma1' },
            { src: KKhuma2, alt: 'Kkhuma2' },
            { src: KKhuma3, alt: 'Kkhuma3' },
            { src: KKhuma4, alt: 'Kkhuma4' },
            { src: KKhuma5, alt: 'Kkhuma5' },
        ]
    },
    {
        name: 'Driak21',
        flag: ukFlag,
        flagAlt: 'uk',
        thumbnail: Driak21Thumbnail,
        images: [
            { src: Driak211, alt: 'Driak211' },
            { src: Driak215, alt: 'Driak215' },
            { src: Driak212, alt: 'Driak212' },
            { src: Driak213, alt: 'Driak213' },
            { src: Driak214, alt: 'Driak214' }
        ]
    },
    {
        name: 'Malelo',
        flag: frenchFlag,
        flagAlt: 'french',
        thumbnail: MaleloThumbnail,
        images: [
            { src: Malelo1, alt: 'Malelo1' },
            { src: Malelo2, alt: 'Malelo2' },
            { src: Malelo3, alt: 'Malelo3' },
            { src: Malelo4, alt: 'Malelo4' },
            { src: Malelo5, alt: 'Malelo5' },
            { src: Malelo6, alt: 'Malelo6' },
            { src: Malelo7, alt: 'Malelo7' }
        ]
    },
    {
        name: 'Nelwyr',
        flag: frenchFlag,
        flagAlt: 'french',
        thumbnail: NelwyrThumbnail,
        images: [
            { src: Nelwyr1, alt: 'Nelwyr1' },
            { src: Nelwyr2, alt: 'Nelwyr2' },
            { src: Nelwyr3, alt: 'Nelwyr3' },
            { src: Nelwyr4, alt: 'Nelwyr4' }
        ]
    },
    {
        name: 'Certtes',
        flag: frenchFlag,
        flagAlt: 'french',
        thumbnail: CerttesThumbnail,
        images: [
            { src: Certtes1, alt: 'Certtes1' },
            { src: Certtes2, alt: 'Certtes2' },
            { src: Certtes3, alt: 'Certtes3' },
            { src: Certtes4, alt: 'Certtes4' },
            { src: Certtes5, alt: 'Certtes5' }
        ]
    },
    {
        name: 'HighQueue',
        flag: frenchFlag,
        flagAlt: 'french',
        thumbnail: HighqueueThumbnail,
        images: [
            { src: Highqueue1, alt: 'HighQueue1' },
            { src: Highqueue2, alt: 'HighQueue2' }
        ]
    },
    {
        name: 'Mugen',
        flag: frenchFlag,
        flagAlt: 'french',
        thumbnail: MugenThumbnail,
        images: [
            { src: Mugen1, alt: 'Mugen1' },
            { src: Mugen2, alt: 'Mugen2' },
            { src: Mugen3, alt: 'Mugen3' }
        ]
    },
    {
        name: 'Khéops',
        flag: frenchFlag,
        flagAlt: 'french',
        thumbnail: kheopsThumbnail,
        images: [
            { src: kheops1, alt: 'kheops1' },
            { src: kheops3, alt: 'kheops3' },
            { src: kheops2, alt: 'kheops2' }
        ]
    },
    {
        name: 'Elyfro',
        flag: frenchFlag,
        flagAlt: 'french',
        thumbnail: elyfroThumbnail,
        images: [
            { src: elyfro1, alt: 'elyfro1' },
            { src: elyfro4, alt: 'elyfro4' },
            { src: elyfro2, alt: 'elyfro2' },
            { src: elyfro3, alt: 'elyfro3' }
        ]
    },
    {
        name: 'Fianso',
        flag: frenchFlag,
        flagAlt: 'french',
        thumbnail: fiansoThumbnail,
        images: [
            { src: fianso1, alt: 'fianso1' },
            { src: fianso5, alt: 'fianso5' },
            { src: fianso2, alt: 'fianso2' },
            { src: fianso3, alt: 'fianso3' },
            { src: fianso4, alt: 'fianso4' }
        ]
    },
    {
        name: 'Light',
        flag: ukFlag,
        flagAlt: 'uk',
        thumbnail: lightThumbnail,
        images: [
            { src: light1, alt: 'light1' },
            { src: light2, alt: 'light2' }
        ]
    },
        {
        name: 'Deadcrafteur',
        flag: frenchFlag,
        flagAlt: 'french',
        thumbnail: DeadcrafteurThumbnail,
        images: [
            { src: Deadcrafteur1, alt: 'Deadcrafteur1' },
            { src: Deadcrafteur2, alt: 'Deadcrafteur2' },
            { src: Deadcrafteur3, alt: 'Deadcrafteur3' },
            { src: Deadcrafteur4, alt: 'Deadcrafteur4' },
            { src: Deadcrafteur5, alt: 'Deadcrafteur5' },
            { src: Deadcrafteur6, alt: 'Deadcrafteur6' }
        ]
    },
    {
        name: 'ElToredo',
        flag: frenchFlag,
        flagAlt: 'french',
        thumbnail: ElToredoThumbnail,
        images: [
            { src: ElToredo1, alt: 'eltoredo1' },
            { src: ElToredo3, alt: 'eltoredo3' },
            { src: ElToredo2, alt: 'eltoredo2' }
        ]
    },
    {
        name: 'Yoshi',
        flag: frenchFlag,
        flagAlt: 'french',
        thumbnail: YoshiThumbnail,
        images: [
            { src: Yoshi1, alt: 'Yoshi1' },
            { src: Yoshi5, alt: 'Yoshi5' },
            { src: Yoshi2, alt: 'Yoshi2' },
            { src: Yoshi3, alt: 'Yoshi3' },
            { src: Yoshi4, alt: 'Yoshi4' }
        ]
    },
    {
        name: 'Psykokwak',
        flag: frenchFlag,
        flagAlt: 'french',
        thumbnail: PsykoThumbnail,
        images: [
            { src: Psyko1, alt: 'Psyko1' },
            { src: Psyko3, alt: 'Psyko3' },
            { src: Psyko2, alt: 'Psyko2' }
        ]
    },
    {
        name: 'Belbarb',
        flag: frenchFlag,
        flagAlt: 'french',
        thumbnail: BelbarbThumbnail,
        images: [
            { src: Belbarb1, alt: 'Belbarb1' },
            { src: Belbarb2, alt: 'Belbarb2' },
            { src: Belbarb3, alt: 'Belbarb3' },
            { src: Belbarb4, alt: 'Belbarb4' },
            { src: Belbarb5, alt: 'Belbarb5' }
        ]
    }
];